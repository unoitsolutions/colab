...
- (void)processBusinessCard:(UIImage*)image withParams:(ProcessingParams*)params
{
    DLOG(@"");
    NSParameterAssert(image);
    
    NSURL* processingURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://cloud.ocrsdk.com/processBusinessCard?%@", [params urlString]]];
...
- (void)uploadFinished:(HTTPOperation*)operation
{
	...
	if (operation.error != nil) {
        ...
        if (task.error != nil) {
            if ([self.delegate respondsToSelector:@selector(client:didFailedWithError:)]) {
                [self.delegate client:self didFailedWithError:task.error];
            }
            return;
        }
...
//    DLOG(@"receivedData: %@",result);
	...
	if (operation.error != nil) {
        ...
        if ([self.delegate respondsToSelector:@selector(client:didFinishDownloadData:)]) {
