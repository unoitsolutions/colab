...
//    NSLog(@"attributeDict: %@",attributeDict);
	if ([elementName isEqualToString:@"task"]) {
		...
		if (self.status == Completed) {
            ...
            self.error = [NSError errorWithDomain:@"ABBYY" code:0 userInfo:@{NSLocalizedDescriptionKey:[attributeDict objectForKey:@"error"]}];
        }
	} else if ([elementName isEqualToString:@"error"]) {
		self.error = [NSError errorWithDomain:@"ABBYY" code:0 userInfo:attributeDict];
	}
}
