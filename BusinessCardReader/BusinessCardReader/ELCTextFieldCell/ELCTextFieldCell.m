...
{    
	if ((self = [super initWithStyle:style reuseIdentifier:reuseIdentifier])) {
		...
		[_leftLabel setFont:[UIFont boldSystemFontOfSize:14]];
		[_leftLabel setTextAlignment:UITextAlignmentRight];
        [_leftLabel setNumberOfLines:2];
...
- (void)layoutSubviews
{
	...
	if (_leftLabel.text != nil) {
		...
		_leftLabel.frame = CGRectMake(origFrame.origin.x, origFrame.origin.y, 75, origFrame.size.height-1);
		_rightTextField.frame = CGRectMake(origFrame.origin.x+80, origFrame.origin.y, origFrame.size.width-85, origFrame.size.height);
	} else {
